...
public final class ReciprocalArraySum {
	...
	private static class ReciprocalArraySumTask extends RecursiveAction {
		...
		private static final int SEQUENCIAL_THRESHOLD = 3500;
		...
		protected void compute() {
			if ((endIndexExclusive - startIndexInclusive) < SEQUENCIAL_THRESHOLD) {
	...
	protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
		...
		for (ReciprocalArraySumTask task : tasks) {
			task.join();
			reciprocalSum += task.getValue();
		}
		return reciprocalSum;

		/*
		 * ReciprocalArraySumTask task = new ReciprocalArraySumTask(0,
		 * input.length, input); task.compute();
		 * 
		 * return task.getValue();
		 */
	}
